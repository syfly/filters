...
public class VideoEncoderCore {
    ...
    public VideoEncoderCore(int width, int height, int bitRate, File outputFile)
        ...
        Log.v(TAG, "startRecording: path" + outputFile.toString());
        startRecording();
    }
...
//        }
	...
	private void startRecording() {
		...
		try {
			mMuxerWrapper = new MediaMuxerWrapper(".mp4", mMuxer, mStartLock);	// if you record audio only, ".m4a" is also OK.
			mMuxerWrapper.addVideoEncoderCount();
			if (true) {
				// for audio capturing
				new MediaAudioEncoder(mMuxerWrapper, null/*mMediaEncoderListener*/);
			}
	...
	private void stopRecording() {
		...
		if (mMuxerWrapper != null) {
			mMuxerWrapper.DeclineVideoEncoderCount();
